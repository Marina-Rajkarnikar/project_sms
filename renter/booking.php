...
<table class="table table-bordered mt-5">
    ...
    $query = "Select booking_id, book_details.user_id, Name, Image, pickupdate, dropoffdate, email,
    DATEDIFF(dropoffdate,pickupdate)*price as vehicle_price, book_details.status
    from `book_details`, `vehicles`
     where book_details.vehicle_id=vehicles.vehicle_id AND book_details.status='pending' AND vehicles.user_id='$user_id'";
$result = mysqli_query($con, $query);
    ...
    <td>
        <form action="renter.php?view_bookings" method="POST">
...
<table class="table table-bordered mt-5">
    ...
    DATEDIFF(dropoffdate,pickupdate)*price as vehicle_price, book_details.status
    from `book_details`, `vehicles`
     where book_details.vehicle_id=vehicles.vehicle_id AND book_details.status='approved' AND vehicles.user_id='$user_id'";
$result = mysqli_query($con, $query);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_array($result)) { ?>
    <tbody class="text-center">
